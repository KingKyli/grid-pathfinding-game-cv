...
Για την παράδοση **δεν** ανεβάζουμε τη βιβλιοθήκη SGG (sources/.lib/.dll) ούτε build artifacts. Υπάρχει έτοιμο script που φτιάχνει καθαρό zip και κρατάει **μόνο** τα 2 απαραίτητα headers της SGG: `sgg/graphics.h` και `sgg/scancodes.h`.

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\make_submission_zip.ps1 -ZipName submission.zip
```

Το zip που παράγεται:
- Περιέχει τον κώδικά σου (`src/`, `include/`, `grid/`, `examples/`, `configs/`, `maps/`, `docs/`, `tools/`).
- Περιέχει **μόνο** `sgg/graphics.h` και `sgg/scancodes.h` (όχι `sgg-main/`, όχι βιβλιοθήκες).
- Δεν περιέχει προσωρινά/μεγάλα αρχεία τύπου `*.pdb`, `*.obj`, `*.ilk`, ούτε φακέλους τύπου `.vs/`, `build*/`.
